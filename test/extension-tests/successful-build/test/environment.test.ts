...
suite('[Environment]', async () => {
  ...
  test('Passing env-vars to CMake but not to the compiler', async () => {
    // Set fake settings
    await testEnv.setting.changeSetting('configureEnvironment', {_CONFIGURE_ENV: '${workspaceRootFolderName}'});
  ...
  test('Passing env-vars to the compiler but not to CMake', async () => {
    // Set fake settings
    await testEnv.setting.changeSetting('buildEnvironment', {_BUILD_ENV: '${workspaceRootFolderName}'});
  ...
  test('Passing env-vars to CMake AND to the compiler', async () => {
    // Set fake settings
    await testEnv.setting.changeSetting('environment', {_ENV: '${workspaceRootFolderName}'});
